...
const SelectionImage = ({text}: {text: Boolean}) => {
  ...
  return (
    <div className={`container grid grid-rows-${devideQuestionGrid()} grid-flow-col gap-3`}>
        {temp.map(el => {
            return (
        <div key={el} className={`border-[2px] ${selectQuestion === el ?  'border-indigo-500':'border-gray-500'}  p-2 cursor-pointer`} onClick={() => setSelectQuestion(el)}>
            ...
            {text && (
                <div className='flex justify-center mt-2 gap-2'>
                    ...
                    <p className='font-normal text-[20px] text-center mt-2'>{'Makanan'}</p>
                    </div>
            )
            }
        </div>
